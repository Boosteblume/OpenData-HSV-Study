...
file_list = [] 
for i in os.listdir(r"weather\Lufttemperatur"):
    if i.endswith(".txt"):
        i = "weather/Lufttemperatur/" + i
        file_list.append(i)
...
def temperatur():
     ...
     cursor.execute("DROP TABLE IF EXISTS temperatur;")
     print("Finished dropping table (if existed)")


     # Create a table
     cursor.execute("CREATE TABLE temperatur (id serial PRIMARY KEY, Jahr VARCHAR(10), Monat VARCHAR(5), Hamburg VARCHAR(10));")
     print("Finished creating table")

     i = 0 
     #Insert Dataframe into SQL Server:
     for index, row in end_df.iterrows():
          cursor.execute("INSERT INTO temperatur (Jahr, Monat, Hamburg) values (%s, %s, %s);", (row['Jahr'], row['Monat'], row['Hamburg']))
